...
pub fn create_mount(
    ...
    insert_scanned_entries(&tx, &mount_id, &scanned_entries)?;
    tx.commit()?;
...
pub fn reconcile_mount(
    ...
    let changed = !mount.is_available
        ...
        || existing_entries != scanned_entries;
    ...
    tx.execute(
        "UPDATE nodes SET state = ?2, name = ?3 WHERE id = ?1",
...
fn load_mount_entries(
    conn: &Connection,
    mount_id: &str,
) -> rusqlite::Result<Vec<ScannedMountEntry>> {
    let mut stmt = conn.prepare(
        "
        SELECT child.relative_path, parent.relative_path, child.name, child.kind
        FROM nodes child
...
fn insert_scanned_entries(
    ...
    for entry in scanned_entries {
        ...
        conn.execute(
            "
            INSERT INTO nodes (id, parent_id, kind, name, state, mount_id, relative_path)
