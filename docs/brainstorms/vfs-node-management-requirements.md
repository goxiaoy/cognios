...
The three Milestone 1 node types cover the core ingestion surface: virtual organization (Folder), web content (URL), and local file access (Mount). Additional node types from the full product vision (System Logs, Local Chat Threads) are deferred to later milestones.
...
User
 ...
 │                                                                    │  (tree)
 └── Mount Dir ──→ Apply Ignore Config ──→ FS Watcher (live sync) ──→┘
```

## Requirements

**Folder Nodes**
- R1. Users can create a Folder node with a name. Folders serve as virtual containers and can be nested within other Folders. Circular references (e.g., Folder A nested inside itself) are rejected by the VFS.
- R2. Folders have no filesystem backing — they exist purely within the VFS.
...
**Mount Nodes**
- R6. Users can mount a local directory as a Mount node. The directory's file tree is mirrored as VFS nodes beneath the mount point.
...
**Node Mutations**
- R16. Users can rename any node (Folder, URL, or Mount). For mounted file and directory nodes, the rename is applied to the underlying filesystem and the VFS mirror reflects the change.
- R17. Users can delete any node. Behavior by type:
  ...
  - **Mounted file or directory node:** The file or directory is deleted from the underlying filesystem; the VFS mirror reflects the removal.

**Explorer**
- R11. The Explorer displays all VFS nodes (Folders, URLs, Mounts, and mounted files) as a navigable tree, updating in real time as the VFS changes (see R9 for mount watching, R5 for URL indexing state transitions).
...
- Adding or deleting a file in a mounted directory is reflected in the Explorer within ~1 second.
...
- Renaming or deleting a mounted file node is reflected both in the Explorer and on disk.
...
- **Milestone 1 node types are Folder, URL, Mount:** State machine is `Pending → Indexing → Indexed / Error`. This is the authoritative M1 taxonomy. The PRD's node types (Local Files, System Logs, Local Chat Threads) and `Verified` state belong to the broader product vision and do not apply to M1 implementation; they will be introduced in later milestones.
...
- **[Affects R15] [Technical]** Persistence backend (likely SQLite) and schema design for VFS nodes, including the parent-child relationship model (whether Folders can contain URL and Mount nodes at the same level).
