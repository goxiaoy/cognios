...
``/models/*`` router. The current runtime uses Rust-owned background
tasks and direct ``/index/*`` calls. Unit 6 adds ``/search``. Phase 1
of the feature-oriented Settings plan adds ``/settings``.
"""
...
from .routes import realtime_voice as realtime_voice_routes
...
def build_app(
    *,
    ...
    Each subsystem is optional so tests can mount only what they need.
    Routes whose dependencies are missing return a typed 500 — same
    surface Rust would see if app construction was misordered.
    """
    ...
    app.include_router(realtime_voice_routes.router)
